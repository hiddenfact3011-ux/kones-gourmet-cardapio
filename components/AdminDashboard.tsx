...
import { X, Save, Plus, Trash2, Camera, ArrowLeft, Settings as SettingsIcon, Tag, Package, Copy, Check, Layout, ShieldAlert, BarChart3, Clock, Eye, EyeOff, Star, TrendingUp, Users, ShoppingBag, Edit3, Image as ImageIcon, RotateCcw, Upload, Layers } from 'lucide-react';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ settings, setSettings, categories, setCategories, products, setProducts, onClose, isAdminLoggedIn, setIsAdminLoggedIn }) => {
  ...
  const resetStats = () => {
    if (confirm('Deseja zerar todas as estatísticas de vendas e visitas?')) {
      const newStats = { salesToday: 0, ordersCount: 0, visits: 0, ticketMedio: 0 };
      setStats(newStats);
      localStorage.setItem('kones_stats', JSON.stringify(newStats));
    }
  };
  ...
  const deleteCategory = async (id: string) => {
    if (!confirm('Excluir categoria? Produtos desta categoria podem não aparecer mais.')) return;
    try {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 px-4">
        <div className="bg-white p-10 rounded-[40px] shadow-2xl w-full max-w-md text-center border-t-[12px] border-red-600">
          ...
          <h2 className="text-3xl font-black mb-8 text-gray-900">Painel Admin</h2>
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="password" title="Senha de acesso" className="w-full border-2 border-gray-100 bg-gray-50 rounded-2xl p-5 outline-none focus:border-red-600 text-center text-3xl tracking-[10px]" placeholder="****" value={password} onChange={e => setPassword(e.target.value)} autoFocus />
            <button className="w-full bg-red-600 text-white font-black py-5 rounded-2xl shadow-xl hover:bg-red-700 transition-all uppercase tracking-widest">Acessar Painel</button>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-['Inter']">
      <header className="bg-zinc-900 text-white p-6 flex items-center justify-between sticky top-0 z-50">
        ...
        <button onClick={() => setIsAdminLoggedIn(false)} className="bg-red-600 px-6 py-2 rounded-xl font-bold text-xs hover:bg-red-700 transition">DESLOGAR</button>
      </header>

      <nav className="flex border-b bg-white sticky top-[80px] z-40 overflow-x-auto hide-scrollbar">
        <button onClick={() => setActiveTab('dashboard')} className={`flex-1 p-5 font-black flex flex-col items-center gap-1 min-w-[100px] transition-all text-[10px] uppercase tracking-widest ${activeTab === 'dashboard' ? 'text-red-600 border-b-4 border-red-600 bg-red-50/10' : 'text-gray-300'}`}><BarChart3 size={18} /> Resumo</button>
        <button onClick={() => setActiveTab('products')} className={`flex-1 p-5 font-black flex flex-col items-center gap-1 min-w-[100px] transition-all text-[10px] uppercase tracking-widest ${activeTab === 'products' ? 'text-red-600 border-b-4 border-red-600 bg-red-50/10' : 'text-gray-300'}`}><Package size={18} /> Produtos</button>
        <button onClick={() => setActiveTab('categories')} className={`flex-1 p-5 font-black flex flex-col items-center gap-1 min-w-[100px] transition-all text-[10px] uppercase tracking-widest ${activeTab === 'categories' ? 'text-red-600 border-b-4 border-red-600 bg-red-50/10' : 'text-gray-300'}`}><Layers size={18} /> Categorias</button>
        <button onClick={() => setActiveTab('hours')} className={`flex-1 p-5 font-black flex flex-col items-center gap-1 min-w-[100px] transition-all text-[10px] uppercase tracking-widest ${activeTab === 'hours' ? 'text-red-600 border-b-4 border-red-600 bg-red-50/10' : 'text-gray-300'}`}><Clock size={18} /> Horários</button>
        <button onClick={() => setActiveTab('settings')} className={`flex-1 p-5 font-black flex flex-col items-center gap-1 min-w-[100px] transition-all text-[10px] uppercase tracking-widest ${activeTab === 'settings' ? 'text-red-600 border-b-4 border-red-600 bg-red-50/10' : 'text-gray-300'}`}><Layout size={18} /> Design</button>
      </nav>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8 animate-slide-in">
             <div className="flex justify-between items-center">
                ...
                <button onClick={resetStats} className="flex items-center gap-2 text-xs font-black text-red-600 bg-red-50 px-4 py-2 rounded-xl">ZERAR TUDO</button>
             </div>
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm">
                   <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Vendas Hoje</p>
                   <p className="text-2xl font-black text-gray-900">R$ {stats.salesToday.toFixed(2)}</p>
                </div>
                <div className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm">
                   <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Pedidos</p>
                   <p className="text-2xl font-black text-gray-900">{stats.ordersCount}</p>
                </div>
                <div className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm">
                   <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Visitas</p>
                   <p className="text-2xl font-black text-gray-900">{stats.visits}</p>
                </div>
             </div>
          </div>
        )}
        ...
        {activeTab === 'categories' && (
           <div className="space-y-8 animate-slide-in">
              <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm space-y-6">
                 <h3 className="text-xl font-black text-gray-900 flex items-center gap-2"><Layers className="text-red-600"/> Categorias do Cardápio</h3>
                 <div className="flex gap-2">
                    <input type="text" placeholder="Ex: Sobremesas" className="flex-1 p-5 bg-gray-50 border-2 border-transparent focus:border-red-600 rounded-[24px] outline-none font-bold" value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} />
                    <button onClick={handleAddCategory} disabled={isSaving} className="bg-red-600 text-white px-8 rounded-[24px] font-black text-xs uppercase shadow-lg shadow-red-200">ADICIONAR</button>
                 </div>
        ...
        {activeTab === 'products' && (
           <div className="space-y-6 animate-slide-in">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setEditingProduct({ id: `temp-${Date.now()}`, name: '', description: '', price: 0, image: '', categoryId: categories[0]?.id || '', extras: [], active: true })} className="bg-red-600 text-white px-6 py-3 rounded-2xl font-black text-xs shadow-lg shadow-red-200"><Plus size={16}/> NOVO PRODUTO</button>
              </div>
                 ...
                 {products.map(p => (
                   <div key={p.id} className="bg-white p-5 rounded-[32px] border border-gray-100 shadow-sm flex items-center gap-6 group">
                      <div className="relative shrink-0 w-20 h-20 overflow-hidden rounded-2xl">
                        <img src={p.image || 'https://via.placeholder.com/150'} className={`w-full h-full object-cover object-center ${!p.active && 'grayscale'}`} />
        ...
        {activeTab === 'settings' && (
           <div className="space-y-8 animate-slide-in">
              <div className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm space-y-8">
                 <h3 className="text-xl font-black text-gray-900 flex items-center gap-2"><ImageIcon className="text-red-600"/> Identidade Visual</h3>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="flex flex-col items-center gap-4">
                       <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Logo da Loja</p>
                       <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-gray-50 shadow-xl relative group bg-gray-100">
                          <img src={settings.logo} className="w-full h-full object-cover object-center" alt="Preview Logo" />
                    ...
                    <div className="flex flex-col items-center gap-4">
                       ...
                       <div className="w-full h-32 rounded-[28px] overflow-hidden border-4 border-gray-50 shadow-xl relative group bg-gray-100">
                          <img src={settings.banner} className="w-full h-full object-cover object-center" alt="Preview Banner" />
              ...
              <button disabled={isSaving} onClick={() => saveSettings(settings)} className="w-full bg-red-600 text-white py-6 rounded-[32px] font-black shadow-xl shadow-red-100 uppercase tracking-widest">SALVAR TUDO</button>
           </div>
        )}
      </main>
        ...
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
           <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-slide-in">
              ...
              <form onSubmit={handleProductSubmit} className="p-8 overflow-y-auto space-y-6 custom-scrollbar">
                 <div className="flex flex-col items-center gap-4 border-b pb-6">
                    <div onClick={() => fileInputRef.current?.click()} className="w-48 h-48 rounded-[32px] overflow-hidden border-4 border-white shadow-2xl bg-gray-50 relative group cursor-pointer">
                       <img src={editingProduct.image || 'https://via.placeholder.com/300?text=Sem+Foto'} className="w-full h-full object-cover object-center" alt="Produto" />
              ...
              <div className="p-8 border-t bg-gray-50">
                 <button type="submit" disabled={isSaving} onClick={handleProductSubmit} className="w-full bg-red-600 text-white py-5 rounded-[28px] font-black shadow-xl shadow-red-200 uppercase tracking-widest">SALVAR PRODUTO</button>
              </div>
           </div>
        </div>
      )}
