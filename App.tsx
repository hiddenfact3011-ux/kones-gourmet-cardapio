...
import { Settings, Search, X, MapPin, Clock, Star, ShoppingBag, Send, ChevronRight, MessageCircle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const sendOrder = () => {
    ...
    let message = `*PEDIDO KONES GOURMET*\n\n`;
    ...
    <div className="min-h-screen bg-gray-50 pb-28 font-['Inter']">
      ...
      <div className="mt-12 px-5 max-w-2xl mx-auto space-y-6">
        ...
        {settings.promotion?.active && (
          <div className="bg-gradient-to-r from-red-600 to-red-800 rounded-[32px] p-1 shadow-lg animate-pulse hover:animate-none cursor-pointer overflow-hidden" onClick={() => setSelectedProduct(products.find(p => p.name === settings.promotion?.name) || null)}>
            <div className="bg-white rounded-[31px] p-4 flex gap-4 items-center">
              ...
              <div className="flex-1">
                <span className="bg-red-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase mb-1 inline-block">Destaque de Hoje</span>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/70 backdrop-blur-sm animate-slide-in">
          <div className="bg-white w-full max-w-xl rounded-t-[40px] max-h-[95vh] flex flex-col overflow-hidden">
            ...
            <div className="p-8 overflow-y-auto space-y-6">
              ...
              {selectedProduct.extras.length > 0 && (
                <div className="space-y-3">
                  ...
                  {selectedProduct.extras.map(e => (
                    <label key={e.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl cursor-pointer border border-transparent hover:border-red-100">
              ...
              <div className="space-y-2">
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Ex: Tirar cebola, pouco molho..." className="w-full p-4 bg-gray-50 rounded-2xl border-2 border-transparent focus:border-red-600/20 outline-none text-sm resize-none h-24" />
              </div>
            </div>
            <div className="p-8 border-t bg-gray-50">
              ...
              }} className="w-full bg-red-600 text-white font-black py-5 rounded-2xl shadow-xl uppercase tracking-widest">Adicionar - R$ {(selectedProduct.price + selectedExtras.reduce((s, id) => s + (selectedProduct.extras.find(e => e.id === id)?.price || 0), 0)).toFixed(2)}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-[110] flex items-end justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xl rounded-t-[40px] max-h-[95vh] flex flex-col">
            <div className="p-6 border-b flex justify-between items-center">
              <h2 className="text-xl font-black">Sua Sacola</h2>
              <button onClick={() => setIsCartOpen(false)} className="p-2"><X /></button>
            </div>
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
                  ...
                  return p ? (
                    <div key={item.id} className="flex justify-between items-start border-b pb-4">
                      ...
                      <div className="text-right">
                        <p className="font-black text-sm text-red-600">R$ {p.price.toFixed(2)}</p>
              ...
              <div className="space-y-4 bg-gray-50 p-6 rounded-[32px]">
                <p className="text-xs font-black text-gray-400 uppercase tracking-widest text-center">Dados para Entrega</p>
                <input placeholder="Seu Nome Completo" className="w-full p-4 rounded-xl border-2 border-white focus:border-red-600/20 outline-none font-bold" value={customerInfo.name} onChange={e => setCustomerInfo({...customerInfo, name: e.target.value})} />
                <input placeholder="Endereço (Rua, Nº, Bairro)" className="w-full p-4 rounded-xl border-2 border-white focus:border-red-600/20 outline-none font-bold" value={customerInfo.address} onChange={e => setCustomerInfo({...customerInfo, address: e.target.value})} />
                <input placeholder="Ponto de Referência" className="w-full p-4 rounded-xl border-2 border-white focus:border-red-600/20 outline-none font-bold" value={customerInfo.ref} onChange={e => setCustomerInfo({...customerInfo, ref: e.target.value})} />
                <div className="flex gap-2">
                  {['PIX', 'DINHEIRO', 'CARTÃO'].map(m => (
                    <button key={m} onClick={() => setCustomerInfo({...customerInfo, payment: m})} className={`flex-1 py-3 rounded-xl font-black text-[10px] border-2 transition ${customerInfo.payment === m ? 'bg-red-600 border-red-600 text-white' : 'bg-white border-transparent text-gray-400'}`}>{m}</button>
                  ))}
                </div>
              </div>

              <div className="space-y-2 border-t pt-4">
                <div className="flex justify-between text-gray-400 text-sm"><span>Subtotal</span><span>R$ {total.toFixed(2)}</span></div>
                <div className="flex justify-between text-gray-400 text-sm"><span>Taxa de Entrega</span><span>R$ {settings.deliveryFee.toFixed(2)}</span></div>
                <div className="flex justify-between font-black text-xl text-gray-900 mt-2"><span>TOTAL</span><span>R$ {(total + settings.deliveryFee).toFixed(2)}</span></div>
              </div>
            </div>
            <div className="p-8 border-t">
              <button onClick={sendOrder} className="w-full bg-red-600 text-white font-black py-5 rounded-2xl shadow-xl flex items-center justify-center gap-3 uppercase tracking-widest active:scale-95 transition">
                <MessageCircle size={24}/> Enviar Pedido para WhatsApp
              </button>
            </div>
          </div>
        </div>
